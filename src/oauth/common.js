import { base64UrlDecode, base64UrlEncode, constantTimeEqual, decoder, encoder } from "../security/encoding.js";
...
export async function fetchFormJson(url, values, fetchFn = fetch) {
  ...
  if (!response.ok) {
    const error = new Error(data.error_description || data.message || data.error || `OAuth provider error (${response.status})`);
