import { cleanName, parseYouTubeUrl } from "../validation/common.js";
...
export function createGroupService({ groupRepository, auditRepository }) {
  return {
    async create(teamId, input) {
      ...
      await auditRepository.record("team-admin", teamId, "group.create", "group", groupId, { name });
      return groupRepository.getPublicById(teamId, groupId);
    },

    async update(teamId, groupId, input) {
      ...
      await auditRepository.record("team-admin", teamId, "group.update", "group", groupId, { name, enabled: Boolean(enabled), sortOrder });
      return groupRepository.getPublicById(teamId, groupId);
    },

    async remove(teamId, groupId) {
      const group = await groupRepository.findById(teamId, groupId);
      if (!group) throw new ServiceError("not_found", "", 404);
      await groupRepository.softDelete(teamId, groupId);
      await auditRepository.record("team-admin", teamId, "group.soft_delete", "group", groupId);
      return { ok: true };
    }
  };
}
