import { OAUTH_COOKIE, USER_COOKIE } from "../config/constants.ts";
import { PRIVACY_VERSION, TERMS_VERSION, legalConsentMatches } from "../config/legal.ts";
import { serviceErrorResponse } from "../http/errors.ts";
import { apiJson, withHeaders } from "../http/response.ts";
import { createPkcePair, oauthStateValues, redirectUri } from "../oauth/common.ts";
import { deauthorizeLineApp, exchangeAndVerifyProvider, OAUTH_PROVIDERS, providerAuthorizationUrl, providerConfigured, providerStatus } from "../oauth/providers.ts";
import { requireLegacyTeamAdmin, requireUser } from "../security/authorization.ts";
...
export async function accountCreateTeam(request, env) {
  ...
  const parsed = await parseJsonBody(request, accountCreateTeamBodySchema);
  ...
  try {
    ...
    const result = await accountService.createTeam(session.userId, parsed.data);
...
export async function accountDelete(request, env, url) {
  const session = await requireUser(request, env);
  if (!session) return apiJson({ error: "unauthorized" }, 401);
  const parsed = await parseJsonBody(request, accountDeleteBodySchema);
  if (parsed.response) return parsed.response;
  if (parsed.data.confirm.trim() !== "退会する") {
