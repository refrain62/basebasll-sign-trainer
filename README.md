...
- LPの情報量を整理し、プラン詳細を `/plans`、PWAインストール手順を `/install` へ分離。
- Free利用中でもPlus / Proの機能導線を鍵付きで残し、クリックすると該当するプラン説明を別タブで開く。
- チーム管理の「プラン・認証」でもロック機能を一覧から確認し、その場から詳細説明へ移動できる。
- `0018_sample_team_groups.sql` でサンプルチームの5グループと既存10サインの割当をdev / staging / productionへ共通投入。

## v1.5.33

- Plusに「今回の練習結果を文章で共有」を追加し、Web Share APIでLINE等へ共有できるようにしました。PC等で共有APIが使えない場合はクリップボードへコピーします。
- Proは文章共有に加え、今回の練習結果・成績分析をブラウザ内で画像カード化して共有できます。画像生成時も成績データをサーバーへ送信しません。
- Proの詳細分析ロジック、Plus/Proの共有ロジックは通常のクライアントbundleへ同梱せず、選手セッションとEntitlementをサーバー側で再確認した後だけ `/api/premium-module` から配信します。URL直打ち・API直叩き・クライアント状態改変だけでは有償モジュールを取得できません。
- `migrations/0017_result_sharing.sql` を追加し、Plus=文章共有、Pro=文章+画像共有のEntitlementを定義しました。
- LPと仕様・運用・設計・テストドキュメントを共有機能と有償モジュール配信方式へ同期しました。
