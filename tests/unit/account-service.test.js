...
test("account deletion is blocked while the user owns a team", async () => {
  ...
  await assert.rejects(() => fx.service.deleteAccount("u_owner123"), (error) => error.code === "owned_teams_remaining" && error.status === 409);
...
test("account deletion detects an ownership race even after the pre-check", async () => {
  ...
  await assert.rejects(() => fx.service.deleteAccount("u_admin123"), (error) => error.code === "owned_teams_remaining");
});
...
test("team creation can require current legal consent", async () => {
  ...
  await assert.rejects(() => fx.service.assertLegalConsent("u_owner123", "2027-01-01", "2026-09-24"), (error) => error.code === "legal_consent_required" && error.status === 428);
});
