...
import { createEntitlementService } from "../../src/services/entitlement-service.js";
...
test("assertFeature blocks unavailable future features with a service error", async () => {
  ...
  await assert.rejects(
    () => service.assertFeature("T1", FEATURE_KEYS.DIRECT_IMAGE_UPLOAD),
    (error) => error.code === "feature_not_available" && error.status === 403 && error.details.feature === FEATURE_KEYS.DIRECT_IMAGE_UPLOAD
  );
});
