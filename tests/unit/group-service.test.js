...
import { createGroupService } from "../../src/services/group-service.js";
...
test("group service rejects invalid YouTube URL before repository mutation", async () => {
  const { service, calls } = fixture();
  await assert.rejects(() => service.create("team1", { name: "A", youtubeUrl: "https://evil.example/video" }), (error) => {
