...
import { createSystemTeamService } from "../../src/services/system-team-service.ts";
...
test("system team service enforces admin password policy independently of HTTP", async () => {
  const { service } = build();
  await expect(service.create({ name: "T", passphrase: "x", adminPassword: "abcdefghijkl" })).rejects.toMatchObject({ code: "weak_admin_password", status: 400 });
});
