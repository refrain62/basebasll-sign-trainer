...
import { createSignService } from "../../src/services/sign-service.ts";
...
test("sign service rolls back newly created sign when initial video URL is invalid", async () => {
  const { service, calls } = makeService();
  await expect(service.create("team1", { name: "サイン", youtubeUrl: "not youtube" })).rejects.toMatchObject({ code: "invalid_youtube" });
